...
using callback_type = std::function<void(const ImDrawList *, const ImDrawCmd *)>;

static void invoke(const ImDrawList *parent_list, const ImDrawCmd *cmd) {
    auto callback = static_cast<callback_type *>(cmd->UserCallbackData);
    (*callback)(parent_list, cmd);
    delete callback;
}
...
int main(int, char **) {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    create_mvp_window(mvp);

    {
      ImGui::Begin("View window", nullptr, ImGuiWindowFlags_NoScrollbar);
      ImGui::BeginChild("image_region", ImVec2(0, -ImGui::GetFrameHeight()));
      auto win_pos = ImGui::GetWindowPos();
      auto win_size = ImGui::GetWindowSize();
      auto callback = new callback_type([win_pos, win_size, mvp](const ImDrawList *parent_list, const ImDrawCmd *cmd) {
        auto &io = ImGui::GetIO();
        glViewport(win_pos.x, io.DisplaySize.y - win_size.y - win_pos.y, win_size.x, win_size.y);
        shader shader;
        shader.render(mvp);
        });
      ImGui::GetWindowDrawList()->AddCallback(invoke, callback);
